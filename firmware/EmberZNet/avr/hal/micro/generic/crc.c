/** @file hal/micro/generic/crc.c
 *  @brief  Generic firmware source for Cyclic Redundancy Check calculations.
 *
 * By defining CCITT, we use the CCITT calculation instead of the CRC16
 * since it requires less code.
 *
 * <!-- Author(s): Lee Taylor, lee@ember.com -->
 * <!-- Author(s): Jeff Mathews, jm@ember.com -->
 * <!-- Copyright 2004 by Ember Corporation. All rights reserved.       *80*-->   
 */

#include PLATFORM_HEADER
#include "stack/include/ember-types.h"
#include "hal/hal.h"

#define CCITT // g(X) = X^16 + X^12 + X^5 + 1
//#define CRC16 // g(X) = X^16 + X^15 + X^5 + 1

/*
  16bit CRC notes:
  "CRC16"
    poly is g(X) = X^16 + X^15 + X^5 + 1  (0x8005)
    used with modbus
    initial remainder should be 0x0000
    data and remainder are reflected
  "CRC-CCITT"
    poly is g(X) = X^16 + X^12 + X^5 + 1  (0x1021)
    used in the FPGA (green boards and 15.4)
    initial remainder should be 0xFFFF
*/


#ifdef CRC16

// i believe these tables are from the modbus protocl spec
static const PGM int8u crc_high[] = {
  
}; 

static const PGM int8u crc_low[] = {
  
};

// this takes about 34 cycles on the mega128 (8.5us @4MHz)
int16u halCommonCrc16(int8u byte, int16u crc) 
{
  int8u high = HIGH_BYTE(crc);
  int8u low = LOW_BYTE(crc);
  int8u i = high ^ byte;
  high = low ^ crc_high[i];
  low = crc_low[i];
  return HIGH_LOW_TO_INT(high,low);
}

#endif // CRC16


#ifdef CCITT

// This CRC seems to take about the same amount of time as the table driven CRC
// which was timed at 34 cycles on the mega128 (8.5us @4MHz).  
int16u halCommonCrc16( int8u byte, int16u crc  )
{
  crc = (crc >> 8) | (crc << 8);
  crc ^= byte;
  crc ^= (crc & 0xff) >> 4;
  crc ^= (crc << 8) << 4;

  // What I wanted is the following function:
  // crc ^= ((crc & 0xff) << 4) << 1;
  // Unfortunately the compiler does this in 46 cycles.  The next line of code
  // does the same thing, but the compiler uses only 10 cycles to implement
  // it. 
  crc ^= ( (int8u) ( (int8u) ( (int8u) (crc & 0xff) ) << 5)) |
    ((int16u) ( (int8u) ( (int8u) (crc & 0xff)) >> 3) << 8);

  return crc;
}

#endif // CCITT
