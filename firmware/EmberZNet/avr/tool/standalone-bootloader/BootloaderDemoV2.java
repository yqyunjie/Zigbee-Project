...
/* sample application and version 2 of standalone bootloader software
/* for both EM250 and EM2420.  
/*
...
public class BootloaderDemoV2 {
  ...
  private static final int BOOTLOADER_MODE_CLONE				= 2;
  private static final int BOOTLOADER_MODE_PASSTHRU_V1          = 3;
  ...
  private static final int BOOTLOAD_PASSTHRU_RECOVERY_V1 = 5;
  private String host;
  ...
  {
	// Reset all sockets
	if (!resetConnection.connect()) {
      debug("Failed to connect to backchannel reset port.");
      return false;
    }
...
/**
  ...
  /**
   * Attempts to bootload the target node using the image running on the
   * attached host.
   *
   * @param targetEuid  ASCII hex representation of the target note's EUI64.
   */

  public void cloneHost(String targetEuid)
  {
    String cloneString = appSerialConnection.expect("clone" 
                                                   + " {" + targetEuid + "}",
                                                   ".*clone remote node.*",
                                                   QUERY_TIMEOUT,
                                                   true);

    System.out.println("Sending clone command... host->" + targetEuid);
    if (cloneString == null) {
      System.out.println("Clone attempt failed.");
    } else {
      System.out.println("Please allow minimum of 30 seconds for cloning to");
      System.out.println("complete, then use \'query_neighbor\' to verify target.");
    }
  }

  
  /**
   * Attempts to recover a failed clone target.  Note that the host node
   * has to be one hop away from the target to do recovery.  The host node
   * sends query broadcast to find failed nodes.  It would recover the first
   * failed node that sends query response reply.  If no target is found within the 
   * query timeout, the host will resume execution of its application.
   *
   */

  public void recoverClone(String target)
  {
    String cloneString = 
      appSerialConnection.expect("recover " + "{" + target + "} " + BOOTLOADER_MODE_CLONE, 
                                 ".*Attempting recovery.*",
                                 QUERY_TIMEOUT,
                                 true);

    System.out.println("Sending recover clone command.");
	System.out.println("Please allow minimum of 30 seconds for cloning to complete");
    if (cloneString == null) {
      // The host is not responding properly.
      System.out.println("Could not invoke bootloader.  Please reset host.");
    } 
  }

  /**
   * Attempts to recover a failed clone target using the default channel.  
   * Note that the host node has to be one hop away order to do recovery 
   * in default channel.  The host node changes its channel to default channel
   * (channel 13) then queries for a failed clone target. If one is found, 
   * the host and target repeat the cloning process. If no target is found 
   * within the query timeout, the host will resume execution of its application.
   *
   */

  public void recoverCloneDefaultChannel()
  {
    String cloneString = 
      appSerialConnection.expect("default " + BOOTLOADER_MODE_CLONE, 
                                 ".*Attempting default channel recovery.*",
                                 QUERY_TIMEOUT,
                                 true);

    System.out.println("Sending default recover clone command.");
	System.out.println("Please allow minimum of 30 seconds for cloning to complete");
    if (cloneString == null) {
      // The host is not responding properly.
      System.out.println("Could not invoke bootloader.  Please reset host.");
  ...
  public static void main(String args[]) 
  {
    ...
    if (args.length == 2
      ...
      bd.makeNeighborQuery(); 
             ...
             && args[1].equalsIgnoreCase("clone")
             && args[2].length() == 16)
      bd.cloneHost(args[2]);
    else if (args.length == 3
             && args[1].equalsIgnoreCase("cloneRecovery")
	 		 && args[2].length() == 16)
      bd.recoverClone(args[2]);
    else if (args.length == 2
             && args[1].equalsIgnoreCase("cloneDefaultChannelRecovery"))
      bd.recoverCloneDefaultChannel();
    else if (args.length == 3
      ...
      bd.bootloadRemoteNodeRecoveryDefaultChannel(args[2]);
             ...
             && args[1].equalsIgnoreCase("remoteBootloadRecoveryV1")
			 && args[2].length() == 16)
      bd.bootloadRemoteNodeRecovery(args[3], args[2], BOOTLOAD_PASSTHRU_RECOVERY_V1);
    else
      printHelp();
  ...
  private static void printHelp()
  {
	...
	System.out.println("\n  query_neighbor");
    System.out.println("\n  clone <target euid>");
    System.out.println("     example: clone B1090600006F0D00");
    System.out.println("\n  cloneRecovery <target euid>");
    System.out.println("\n  cloneDefaultChannelRecovery");
    System.out.println("\n  localBootload <filename>");
    System.out.println("     example: localBootload image.ebl");
    System.out.println("\n  remoteBootload <target euid> <filename>");
    System.out.println("     example: remoteBootload B1090600006F0D00 image.ebl or image.bin");
	...
	System.out.println("\n  remoteBootloadRecoveryV1 <target euid> <filename>");
    System.out.println("     example: remoteBootload B1090600006F0D00 image.bin");
	System.out.println();
  ...
  private boolean bootloadFromFile(String imageFileName, String target, int type)
  {
	...
	case BOOTLOAD_PASSTHRU_RECOVERY_V1:
      responseString = appSerialConnection.expect("recover " 
												  + "{" + target + "} "
												  + BOOTLOADER_MODE_PASSTHRU_V1,
                                                  ".*Attempting recovery.*",
                                                  QUERY_TIMEOUT,
                                                  true);
      break;
  ...
  private boolean configureAppBootloadPort()
  {
	...
	match = adminConnection.expect("config", subMatch("EM250"));
	if(match != null) {
	    rate = 115200;	// EM250 bootload baud rate
	} else {
		rate = 38400;	// EM2420 bootload baud rate
	}
