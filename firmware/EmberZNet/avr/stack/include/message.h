 ...
 * - ::EMBER_NETWORK_BUSY - Insufficient resources avalable
 ...
 * setting the profer flag in the network frame control field.
 ...
 * @param sourceEui       The EUI64 of the node that initated the route record.
 ...
 * by the parent becuase the indirect transaction timer expired.
...
/** @brief Cancels an outgoing message.
 *
 * @param message  A message.
 *
 ...
 * network headers.  
 *
 * The first two bytes are interpreted as the 802.15.4 frame
 * control field.  If the Ack Request bit is set, the packet will be retried as
 * necessary.  Completion is reported via ::emberRawTransmitCompleteHandler().
 * Note that the sequence number specified in this packet is transmitted without
 * modification, for proper messaging behavior sequence numbers of consecutive
 * raw packets should be seeded with a random number and incremented from there.
 *
 ...
 *   -# No route available.
 *   -# Insufficient resources avalable in Network or MAC layers to send
 *      message.
 */
