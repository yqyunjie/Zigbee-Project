...
// ashSerialWriteByte() or ashSerialReadByte()
...
// Define the termios bit fields modified by ashSerialInit
...
static int8u outBuffer[MAX_OUT_BLOCK_LEN];  // array to buffer output
static int8u *outBufRd;                     // outBuffer read pointer                    
static int8u *outBufWr;                     // outBuffer write pointer
static int16u outBlockLen;                  // bytes to buffer before writing
static int8u inBuffer[MAX_IN_BLOCK_LEN];    // array to buffer input
static int8u *inBufRd;                      // inBuffer read pointer
static int8u *inBufWr;                      // inBuffer write pointer
static int16u inBlockLen;                   // bytes to read ahead
...
static EzspStatus ashInternalSerialInit(boolean ignoreErrors);
...
EzspStatus ashSerialInit(void)
{
  ...
  status = ashInternalSerialInit(FALSE);
...
static EzspStatus ashInternalSerialInit(boolean ignoreErrors)
{
  ...
  bps = ashReadConfig(baudRate);
  stopBits = ashReadConfig(stopBits);
  rtsCts = ashReadConfig(rtsCts);
  errStr[0] = '\0';

  outBufRd = outBuffer;
  outBufWr = outBuffer;
  outBlockLen = ashReadConfig(outBlockLen);
  if (outBlockLen > MAX_OUT_BLOCK_LEN) {
    outBlockLen = MAX_OUT_BLOCK_LEN;
  }
  inBufRd = inBuffer;
  inBufWr = inBuffer;
  inBlockLen = ashReadConfig(inBlockLen);
  ...
  while (TRUE) { // break out of while on any error
    ...
    if (i < BAUD_TABLE_SIZE) {
      ...
      snprintf(errStr, ERR_LEN, "Invalid baud rate %d bps\r\n", bps);
      break;
    }
    if ((stopBits != 1) && (stopBits != 2)) {
      snprintf(errStr, ERR_LEN, "Invalid number of stop bits\r\n");
      break;
    }

    ashSerialClose();

    strncpy(port, ashReadConfig(serialPort), ASH_PORT_LEN);
    serialFd = open(port, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (serialFd == NULL_FILE_DESCRIPTOR) {
      snprintf( errStr, ERR_LEN, "Serial port %s open failed: %s\r\n", 
                port, strerror(errno));
      break;
    }
    ...
    // ASH cannot function reliably with blocking serial I/O.
    //
    ...
    tcsetattr(serialFd, TCSAFLUSH, &tios);  // set ASH serial port options
    tcgetattr(serialFd, &checkTios);      // and read back the result

    // Verify that the fields written have the right values
    i = (tios.c_cflag ^ checkTios.c_cflag) & CFLAG_MASK;
    if (i) {
      snprintf(errStr, ERR_LEN, "Termios cflag(s) in error: 0x%04X\r\n", i);
      break;
    }  
    i = (tios.c_iflag ^ checkTios.c_iflag) & IFLAG_MASK;
    if (i) {
      snprintf(errStr, ERR_LEN, "Termios iflag(s) in error: 0x%04X\r\n", i);
      break;
    }  
    i = (tios.c_lflag ^ checkTios.c_lflag) & LFLAG_MASK;
    if (i) {
      snprintf(errStr, ERR_LEN, "Termios lflag(s) in error: 0x%04X\r\n", i);
      break;
    }  
    i = (tios.c_oflag ^ checkTios.c_oflag) & OFLAG_MASK;
    if (i) {
      snprintf(errStr, ERR_LEN, "Termios oflag(s) in error: 0x%04X\r\n", i);
      break;
    }  
    for (i = 0; i < NCCS; i++) {
    ...
    if (i != NCCS) {
      snprintf(errStr, ERR_LEN, "Termios error at c_cc[%d]\r\n", i);
      break;
    }
    if (  (cfgetispeed(&checkTios) != baud) || 
          (cfgetospeed(&checkTios) != baud)) {
      snprintf(errStr, ERR_LEN, "Could not set baud rate to %d bps\r\n", bps);
      break;
    }
    return EZSP_SUCCESS;
  } // while (TRUE)
  ...
  if (strlen(errStr)) {
    ashTraceEvent(errStr);
  }
  ashSerialClose();
  ashError = EZSP_ASH_ERROR_SERIAL_INIT;
...
{
  int8u byte;
  while (ashSerialReadByte(&byte) == EZSP_SUCCESS)
      ;
} 
