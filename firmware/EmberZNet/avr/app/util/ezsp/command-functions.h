...
{
  int8u status;
  startCommand(EZSP_SET_RAM);
  appendInt8u(startIndex);
  appendInt8u(dataLength);
  appendInt8uArray(dataLength, data);
  sendCommand();
  status = fetchInt8u();
...
{
  int8u status;
  startCommand(EZSP_GET_RAM);
  appendInt8u(startIndex);
  appendInt8u(length);
  sendCommand();
  status = fetchInt8u();
  *dataLength = fetchInt8u();
  fetchInt8uArray(*dataLength, data);
  return status;
}
...
{
  int32u time;
  startCommand(EZSP_GET_MILLISECOND_TIME);
  sendCommand();
  time = fetchInt32u();
  return time;
}
...
{
  int8u status;
  startCommand(EZSP_SERIAL_WRITE);
...
{
  int8u messageLength;
  startCommand(EZSP_SERIAL_READ);
  appendInt8u(length);
  sendCommand();
  messageLength = fetchInt8u();
  fetchInt8uArray(messageLength, messageContents);
...
{
  startCommand(EZSP_SCAN_AND_FORM_NETWORK);
  appendInt32u(channelMask);
  appendInt8u(radioTxPower);
  appendInt8uArray(8, extendedPanIdDesired);
  sendCommand();
}
...
void ezspScanAndJoinNetwork(
      ...
      int8u *extendedPanIdDesired)
{
  startCommand(EZSP_SCAN_AND_JOIN_NETWORK);
  appendInt8u(nodeType);
  appendInt32u(channelMask);
  appendInt8u(radioTxPower);
  appendInt8uArray(8, extendedPanIdDesired);
...
void ezspScanForJoinableNetwork(
      ...
      int8u *extendedPanId)
{
  startCommand(EZSP_SCAN_FOR_JOINABLE_NETWORK);
  appendInt32u(channelMask);
...
static void callbackDispatch(void)
{
  ...
  case EZSP_INCOMING_ROUTE_RECORD_HANDLER: {
    ...
    relayList = fetchInt16uPointer(relayCount);
