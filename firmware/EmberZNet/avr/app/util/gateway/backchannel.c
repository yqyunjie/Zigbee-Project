// File: backchannel.c
//
// Description: This code simulates an ISA backchannel serial interface for a
...
const boolean backchannelSupported = TRUE;
...
#define SERVER_PORT_OFFSET 4900
...
static int willConnectionBlock(boolean server, int8u port, boolean read);
...
static boolean getNewConnection(int8u port);
...
EmberStatus backchannelStartServer(int8u port)
{
  ...
  if (bind(socketFd[port],
    ...
    unixError("Error: Could not bind socket to %d", SERVER_PORT_OFFSET + port);
    return EMBER_ERR_FATAL;
  }
  ...
  infoPrint("Listening for connections on port %d", SERVER_PORT_OFFSET + port);
...
EmberStatus backchannelStopServer(int8u port)
{
  ...
  if (clientFd[port] != INVALID_FD) {
    myPrintf(REMOTE_STDOUT, "Server closing client connection.\n");
    close(clientFd[port]);
    clientFd[port] = INVALID_FD;
  }
  if (socketFd[port] != INVALID_FD) {
    myPrintf(LOCAL_STDOUT, "Server closing socket connection %d\n",
                                            ...
                                            boolean waitForConnection)
{
  ...
  if (waitForConnection) {
    infoPrint("Waiting for client connection on port %d", 
...
static boolean getNewConnection(int8u port)
{
  ...
  if (clientFd[port] >= 0) {
    ...
    infoPrint("New connection from client %s:%d",
