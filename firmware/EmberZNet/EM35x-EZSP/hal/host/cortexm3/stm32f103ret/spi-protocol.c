...
int8u halNcpSpipBuffer[SPIP_BUFFER_SIZE];
...
int8u *halNcpFrame = halNcpSpipBuffer + EZSP_LENGTH_INDEX;
...
boolean halNcpClearToTransmit = FALSE;
...
boolean halNcpHostIntAssertedOnPowerup = FALSE;
...
static int8u spiWriteRead(int8u data)
{
  SPI_I2S_SendData(SPI1, data);
  while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET) {}
  data = (int8u)SPI_I2S_ReceiveData(SPI1);
  return data;
}
