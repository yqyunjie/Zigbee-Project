...
static void uartInit(int32u baudRate,
                     int16u wordLength,
                     int16u parity,
                     int16u stopBits)
{
...
size_t __write(int handle, const unsigned char * buffer, size_t size)
{
  ...
  if(forcedWrite) {
    ...
    for(i=0;i<size;i++) {
      ...
      USART_SendData(COM_USART, (int8u)buffer[i]);
      nChars++;
    }
...
int16u halInternalPrintfWriteAvailable()
{
  return ((TRANSMIT_QUEUE_SIZE-1)-txUsed);
}

int16u halInternalPrintfReadAvailable()
{
  return rxUsed;
}
...
void COM_IRQHandler(void)
{
  ...
  if(USART_GetITStatus(COM_USART, USART_IT_RXNE) == SET) {
    int8u incoming = USART_ReceiveData(COM_USART)&0xFF;
    //IAR standard library doesn't allow for any error handling in the
