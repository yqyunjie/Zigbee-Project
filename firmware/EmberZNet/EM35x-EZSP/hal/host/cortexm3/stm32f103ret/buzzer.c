/** @file hal/host/cortexm3/stm32f103ret/buzzer.c
...
void halPlayTune_P(int8u PGM *tune, boolean bkg)
{
  ...
  currentDuration = ((100*((int16u)tune[0]))*((int16u)tune[1]))/125;
  ...
  if(currentDuration-- == 0) {
    if(currentTune[tunePos+1]) {
      if(currentTune[tunePos]) {
        ...
        //Magic duration calculation based on frequency
        //determine how many timer toggles of this note fit in 100ms
        currentDuration = ((100*((int16u)currentTune[tunePos]))*
                            ((int16u)currentTune[tunePos+1]))/125;
      } else {
