...
#include "messaging-client-callback.h"
...
{
  int8u ep = emberAfFindClusterClientEndpointIndex(endpoint, ZCL_MESSAGING_CLUSTER_ID);
...
void emberAfMessagingClusterClientTickCallback(int8u endpoint)
{
  ...
  if (messageTable[ep].active
    ...
    emberAfScheduleClusterTick(endpoint,
                               ...
                               ((messageTable[ep].endTime - messageTable[ep].startTime)
