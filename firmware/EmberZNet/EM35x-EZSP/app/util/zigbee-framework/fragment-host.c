...
{
  EmberStatus status;
  int16u remainingLength = messageLength;
  ...
  while (remainingLength > 0 && totalTxBlocks < MAX_TOTAL_BLOCKS) {
    if (remainingLength > maxFragmentSize) {
      ...
      txFragments[totalTxBlocks] = (int8u)remainingLength;
    }
...
static EmberStatus sendNextFragments(void)
{
  ...
  if (blocksInTransit == 0) {
    fragmentOutgoingType = 0xFF;
    ezspFragmentMessageSentHandler(EMBER_SUCCESS);
  }
...
static void abortTransmission(EmberStatus status)
{
  if (status != EMBER_SUCCESS && fragmentOutgoingType != 0xFF) {
    fragmentOutgoingType = 0xFF;
    ezspFragmentMessageSentHandler(status);
  }
}
                                    ...
                                    int8u **messageContents)
{
  ...
  if (blockNumber == expectedRxBlocks - 1
      || (blockMask | lowBitMask(blockNumber % windowSize)) == 0xFF) {
