...
static void handleBackchannelConnection(int8u port);
...
static boolean readyForSerialInput(int8u port)
{
...
// we are (i.e. the command 'zcl global read <int16u> <int16u>' 
...
char* singleCommandCompletion(const char *text, int state)
{
  ...
  if (commandArgumentPtr
    ...
    // which is what we want.  We can't complete the arbitrary int16u
    // and int8u types, but we do want to display what's next.
    return (char*)NULL;
  }
