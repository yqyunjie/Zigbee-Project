...
EzspStatus ashSetupSerialPort(int* serialPortFdReturn,
                              ...
                              boolean bootloaderMode)
{
  ...
  speed_t baud;
  boolean flowControl = !bootloaderMode;

  bps = (bootloaderMode ? BOOTLOADER_BAUD_RATE : ashReadConfig(baudRate));
  stopBits = (bootloaderMode ? BOOTLOADER_STOP_BITS : ashReadConfig(stopBits));
  ...
  while (TRUE) { // break out of while on any error
    ...
    *serialPortFdReturn = open(ashHostConfig.serialPort, 
    ...
    tcsetattr(*serialPortFdReturn, TCSAFLUSH, &tios);  // set ASH serial port options
...
EzspStatus ashSerialReadByte(int8u *byte)
{
  ...
  status = ashSerialReadAvailable(&count);
  if (status == EZSP_SUCCESS) {
    *byte = *inBufRd++;
