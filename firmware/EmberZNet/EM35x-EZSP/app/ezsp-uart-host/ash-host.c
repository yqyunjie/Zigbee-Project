...
boolean ncpSleepEnabled;                    // ncp is enabled to sleep
...
EzspStatus ashWakeUpNcp(boolean init)
{
  ...
  if (ashSerialReadAvailable(&bytes) == EZSP_SUCCESS) {
    return EZSP_SUCCESS;
  }
...
boolean ashOkToSleep(void)
{
  ...
  return  
    (
      ...
      (ashSerialReadAvailable(&count) == EZSP_ASH_NO_RX_DATA) && // no rx data
      ashQueueIsEmpty(&rxQueue) &&        // no rx frames to process
      ...
      ashQueueIsEmpty(&txQueue) &&        // nothing waiting to transmit
      ashSerialOutputIsIdle()             // nothing in OS buffers or UART FIFO
    );   
}
