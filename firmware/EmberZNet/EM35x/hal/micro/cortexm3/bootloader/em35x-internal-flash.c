...
// We need to use the basepri manipulation functions that aren't brought into the
// bootloader usually so extern them up here.
...
#define BYPASS_MPU_BOOT(blah)               \
  {                                         \
   int32u MPU_CTRL_SAVED = MPU_CTRL_REG;    \
...
int32u internalStorageSize(void)
{
  int32u overhead, size;
  ...
  if ( (halAppAddressTable.baseTable.type) == APP_ADDRESS_TABLE_TYPE ) {
    if( ((halAppAddressTable.baseTable.version & AAT_MAJOR_VERSION_MASK) == AAT_MAJOR_VERSION) && 
      ...
      overhead = 0;
      size = MFB_TOP - (int32u)halAppAddressTable.internalStorageBottom + 1;
    } else {
    ...
    size = (MFB_SIZE_B - BOOTRO_REGION_SIZE)/2;
...
int32u flashAddressFromOffset(int32u address) 
{
  int32u offset = address & (~MFB_PAGE_MASK_B);
  // Always subtract by an extra MFB_PAGE_SIZE_B value since page 0 begins
  // MFB_PAGE_SIZE_B away from the top of flash.
  int32u pageStart = (address&MFB_PAGE_MASK_B) + MFB_PAGE_SIZE_B;
...
{
  int32u endAddr = flashAddressFromOffset(address+length);
  int32u storageSize = internalStorageSize();
...
int8u halEepromRead(int32u address, int8u *data, int16u totalLength)
{
  ...
  if(address&(~MFB_PAGE_MASK_B)) {
    int16u size = MFB_PAGE_SIZE_B - (address&(~MFB_PAGE_MASK_B));
    if(size > totalLength) {
      size = totalLength;
    }
...
int8u halInternalFlashWriteWrapper(int32u address, int16u *data, int16u size)
{
  int8u res;
  int8u savedBasePri;

  // Go ATOMIC using the _disableBasePri function which manipulates BASEPRI
  savedBasePri = _disableBasePri();
  ...
  // Allow interrupts again by clearing BASEPRI
  _writeBasePri(savedBasePri);
...
int8u halEepromWrite(int32u address, const int8u *data, int16u totalLength)
{
  ...
  if(address&(~MFB_PAGE_MASK_B)) {
    ...
    result = halInternalFlashWriteWrapper(address, (int16u*)data, size/2);
    // the address computed is only valid if (totalLength-size) > 0
    address = (address&MFB_PAGE_MASK_B)-MFB_PAGE_SIZE_B; 
    data += size;
    totalLength -= size;
  ...
  if(result == EMBER_SUCCESS) {
    while(totalLength > MFB_PAGE_SIZE_B) {
      result = halInternalFlashWriteWrapper(address, (int16u*)data, MFB_PAGE_SIZE_B/2);
      if(result != EMBER_SUCCESS) {
        break;
      }
      address -= MFB_PAGE_SIZE_B;
  ...
  if((result == EMBER_SUCCESS) && (totalLength > 0)) {
    result = halInternalFlashWriteWrapper(address, (int16u*)data, totalLength/2);
  }
...
// doesn't define these macros forcing us to do it ourselves.
int8u halInternalFlashEraseWrapper(int8u eraseType, int32u address)
{
  int8u res;
  int8u savedBasePri;
