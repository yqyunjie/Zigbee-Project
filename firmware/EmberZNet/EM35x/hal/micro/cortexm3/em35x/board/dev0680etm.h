 ...
 * 
 * <!-- Copyright 2008-2012 by Ember Corporation. All rights reserved.   *80*-->
 */
 ...
 * IRQ is used for VBUS Monitoring.
 *
 ...
 * 
 * VBUS Monitoring is not a requirement for USB to function.  It's only required
 * for properly supporting USB Compliance.  Leaving VBUSMON undefined will
 * keep VBUS Monitoring functionality from being compiled in and not
 * conflict with other pieces that might want to use the GPIO or IRQ that
 * VBUS Monitoring needs.
 */
//#define VBUSMON  PORTA_PIN(3)
/**
 * @brief The GPIO input register for VBUSMON.
 */
#define VBUSMON_IN          GPIO_PCIN
/**
 ...
 * support on PA6 as an input, replacing use of this pin as an LED
 * output.
 *
 * @note This define will override other settings for PA6.
 *
 ...
 * Remember that other pieces that might want to use IRQD.
 * @note For this board, IRQD is pointed at PA6
 */
 ...
 */
#define PWRUP_CFG_LED_RHO_FOR_RHO   GPIOCFG_IN_PUD
#define PWRUP_OUT_LED_RHO_FOR_RHO   GPIOOUT_PULLDOWN /* Deassert */
#define PWRDN_CFG_LED_RHO_FOR_RHO   GPIOCFG_IN_PUD
#define PWRDN_OUT_LED_RHO_FOR_RHO   GPIOOUT_PULLDOWN /* Deassert */
#define WAKE_ON_LED_RHO_FOR_RHO     TRUE

/** @brief Configuration of GPIO for LED operation
 */
#define PWRUP_CFG_LED_RHO_FOR_LED   GPIOCFG_OUT
#define PWRUP_OUT_LED_RHO_FOR_LED   1 /* LED default off */
#define PWRDN_CFG_LED_RHO_FOR_LED   GPIOCFG_OUT
#define PWRDN_OUT_LED_RHO_FOR_LED   1 /* LED off */
#define WAKE_ON_LED_RHO_FOR_LED     FALSE
 ...
 */
  ...
  #define PWRUP_CFG_LED_RHO           PWRUP_CFG_LED_RHO_FOR_RHO
  #define PWRUP_OUT_LED_RHO           PWRUP_OUT_LED_RHO_FOR_RHO
  #define PWRDN_CFG_LED_RHO           PWRDN_CFG_LED_RHO_FOR_RHO
  #define PWRDN_OUT_LED_RHO           PWRDN_OUT_LED_RHO_FOR_RHO
  #define WAKE_ON_LED_RHO             WAKE_ON_LED_RHO_FOR_RHO
  #define halInternalInitRadioHoldOff() halSetRadioHoldOff(TRUE)
#else//!(defined(RADIO_HOLDOFF) && defined(RHO_GPIO))
  // Initial bootup configuration is for LED
  #define PWRUP_CFG_LED_RHO           PWRUP_CFG_LED_RHO_FOR_LED
  #define PWRUP_OUT_LED_RHO           PWRUP_OUT_LED_RHO_FOR_LED
  #define PWRDN_CFG_LED_RHO           PWRDN_CFG_LED_RHO_FOR_LED
  #define PWRDN_OUT_LED_RHO           PWRDN_OUT_LED_RHO_FOR_LED
  #define WAKE_ON_LED_RHO             WAKE_ON_LED_RHO_FOR_LED
  #define halInternalInitRadioHoldOff() /* no-op */
#endif//(defined(RADIO_HOLDOFF) && defined(RHO_GPIO))

#ifdef  RHO_GPIO
  #define WAKE_ON_LED_RHO_VAR         halInternalWakeOnLedOrRho
  extern int8u WAKE_ON_LED_RHO_VAR;
  #define ADJUST_GPIO_CONFIG_LED_RHO(enableRadioHoldOff)  do {                \
          ATOMIC( /* Must read-modify-write so to be safe, use ATOMIC() */    \
            if (enableRadioHoldOff) { /* Radio HoldOff */                     \
              /* Actual register state */                                     \
              /*halGpioConfig(RHO_CFG, PWRUP_CFG_LED_RHO_FOR_RHO);*/          \
              RHO_CFG = RHO_CFG                                               \
                & ~(0x000F                   << ((RHO_GPIO&3)*4))             \
                | (PWRUP_CFG_LED_RHO_FOR_RHO << ((RHO_GPIO&3)*4));            \
              RHO_OUT = RHO_OUT                                               \
                & ~(0x0001                   << ((RHO_GPIO&7)  ))             \
                | (PWRUP_OUT_LED_RHO_FOR_RHO << ((RHO_GPIO&7)  ));            \
              ...
              gpioCfgPowerDown[RHO_GPIO>>2] = gpioCfgPowerDown[RHO_GPIO>>2]   \
                & ~(0x000F                   << ((RHO_GPIO&3)*4))             \
                | (PWRDN_CFG_LED_RHO_FOR_RHO << ((RHO_GPIO&3)*4));            \
              gpioOutPowerDown[RHO_GPIO>>3] = gpioOutPowerDown[RHO_GPIO>>3]   \
                & ~(0x0001                   << ((RHO_GPIO&7)  ))             \
                | (PWRDN_OUT_LED_RHO_FOR_RHO << ((RHO_GPIO&7)  ));            \
              WAKE_ON_LED_RHO_VAR = WAKE_ON_LED_RHO_FOR_RHO;                  \
            } else { /* LED */                                                \
              /* Actual register state */                                     \
              /*halGpioConfig(RHO_CFG, PWRUP_CFG_LED_RHO_FOR_LED);*/          \
              RHO_CFG = RHO_CFG                                               \
                & ~(0x000F                   << ((RHO_GPIO&3)*4))             \
                | (PWRUP_CFG_LED_RHO_FOR_LED << ((RHO_GPIO&3)*4));            \
              RHO_OUT = RHO_OUT                                               \
                & ~(0x0001                   << ((RHO_GPIO&7)  ))             \
                | (PWRUP_OUT_LED_RHO_FOR_LED << ((RHO_GPIO&7)  ));            \
              /* Shadow register state */                                     \
              gpioCfgPowerUp[RHO_GPIO>>2] = gpioCfgPowerUp[RHO_GPIO>>2]       \
                & ~(0x000F                   << ((RHO_GPIO&3)*4))             \
                | (PWRUP_CFG_LED_RHO_FOR_LED << ((RHO_GPIO&3)*4));            \
              ...
              gpioCfgPowerDown[RHO_GPIO>>2] = gpioCfgPowerDown[RHO_GPIO>>2]   \
                & ~(0x000F                   << ((RHO_GPIO&3)*4))             \
                | (PWRDN_CFG_LED_RHO_FOR_LED << ((RHO_GPIO&3)*4));            \
              gpioOutPowerDown[RHO_GPIO>>3] = gpioOutPowerDown[RHO_GPIO>>3]   \
                & ~(0x0001                   << ((RHO_GPIO&7)  ))             \
                | (PWRDN_OUT_LED_RHO_FOR_LED << ((RHO_GPIO&7)  ));            \
              WAKE_ON_LED_RHO_VAR = WAKE_ON_LED_RHO_FOR_LED;                  \
            }                                                                 \
          )} while (0)
#else//!RHO_GPIO
  #define WAKE_ON_LED_RHO_VAR         WAKE_ON_LED_RHO_FOR_LED
#endif//RHO_GPIO
 ...
 */
  ...
  #define PWRUP_CFG_ENUM_CTRL  GPIOCFG_OUT
  #define PWRUP_OUT_ENUM_CTRL  0
  ...
  #define PWRDN_CFG_ENUM_CTRL  GPIOCFG_OUT
  #define PWRDN_OUT_ENUM_CTRL  0
  ...
  #define PWRUP_CFG_USBDP      GPIOCFG_IN
  #define PWRUP_OUT_USBDP      0
  #define PWRUP_CFG_ENUM_CTRL  GPIOCFG_OUT_ALT
  #define PWRUP_OUT_ENUM_CTRL  0
  ...
  #define PWRDN_CFG_ENUM_CTRL  GPIOCFG_IN_PUD
  #define PWRDN_OUT_ENUM_CTRL  GPIOOUT_PULLUP
...
int32u gpioRadioPowerBoardMask = 0
...
int16u gpioCfgPowerDown[6] = {                                                  \
                              ((PWRDN_CFG_USBDM    <<PA0_CFG_BIT)|              \
                               ...
                               (PWRDN_CFG_ENUM_CTRL<<PA2_CFG_BIT)|              \
...
#define WAKE_ON_PA6   WAKE_ON_LED_RHO_VAR
