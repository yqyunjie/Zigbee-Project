...
void setSecurityCommand(void)
{
  ...
  int32u securityBitmask = emberUnsignedCommandArgument(0);

  securityState.bitmask = (int16u) (securityBitmask & 0xFFFF);
  ...
  if ( status == EMBER_SUCCESS 
    ...
    if (status == EMBER_SUCCESS) {
      int16u extendedBitmask = (int16u) (securityBitmask >> 16);
...
{
  int8u nwkIndex = emberUnsignedCommandArgument(0);
