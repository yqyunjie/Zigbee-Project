...
/** @brief This function sends a unicast update of the network key
...
#else
EmberStatus emSendNetworkKeySwitch(EmberNodeId destination);
...
/** @brief This sends an APS remove device command to the destination.
 *    If the destination is an end device then, this must be sent to
 ...
 *    being asked to leave, then those parameters will be the same.
